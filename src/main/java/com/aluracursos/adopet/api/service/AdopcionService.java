...
public class AdopcionService {
    ...
    public void solicitar(SolicitudAdopcionDto dto) {
        ...
        emailService.enviarEmail(
                mascota.getRefugio().getEmail(),
                "Solicitación de adopción",
                "Hola " + mascota.getRefugio().getNombre() +
                        "!\n\nUna solicitud de adopción fue registrada hoy para la mascota: " +
    ...
    public void aprobar(AprobacionAdopcionDto dto) {
        ...
        emailService.enviarEmail(
                ...
                "Felicitaciones " + adopcion.getTutor().getNombre() +
                        "!\n\nSu adopción de la mascota " + adopcion.getMascota().getNombre() +
                        ", solicitada el dia " +
                        adopcion.getFecha().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss")) +
                        ", fue aprobada.\nPor favor, entrar en contacto con el refugio " +
                        adopcion.getMascota().getRefugio().getNombre() +" para ir a buscar a su mascota."
        );
    }
    ...
    public void reprobar(ReprobacionAdopcionDto dto) {
        ...
        adopcion.setJustificativaStatus(dto.justificacion());

        emailService.enviarEmail(
                adopcion.getTutor().getEmail(),
                "Adopción reprobada",
                "Hola " + adopcion.getTutor().getNombre() +
                        "!\n\nInfelizmente su adopción de la mascota " +
                        adopcion.getMascota().getNombre() +", solicitada el dia " +
                        adopcion.getFecha().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss")) +
                        ", fue reprobada por el refugio " + adopcion.getMascota().getRefugio().getNombre() +
                        " con la seguiente justificativa: " + adopcion.getJustificativaStatus()
        );
    }
}
